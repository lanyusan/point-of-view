...
function fastifyView(fastify, opts, next) {
  ...
  const getTemplate = function (file, callback) {
    ...
    if (data && prod) {
      ...
      readFile(join(templatesDir, file), "utf-8", (err, data) => {
        ...
        if (
          options.useHtmlMinifier &&
          typeof options.useHtmlMinifier.minify === "function"
        ) {
          data = options.useHtmlMinifier.minify(
            data,
  ...
  function readCallback(that, page, data) {
    return function _readCallback(err, html) {
      ...
      if (
        ...
        cachedPage = options.useHtmlMinifier.minify(
          cachedPage,
          options.htmlMinifierOptions || {}
        );
      }
  ...
  function preProcessDot(templatesDir, options) {
    ...
    for (const file of readdirSync(destinationDir, { withFileTypes: false })) {
      renderer[basename(file, ".js")] = require(resolve(
  ...
  function viewNunjucks(page, data) {
    ...
    env.render(join(templatesDir, page), data, (err, html) => {
      ...
      if (
        ...
        html = options.useHtmlMinifier.minify(
          html,
          options.htmlMinifierOptions || {}
        );
      }
  ...
  function viewMarko(page, data, opts) {
    ...
    const template =
      opts && opts.templateSrc
        ? engine.load(join(templatesDir, page), opts.templateSrc)
        : engine.load(join(templatesDir, page));
      ...
      if (typeof options.useHtmlMinifyStream === "function") {
        this.send(
          template
            .stream(data)
            .pipe(
              options.useHtmlMinifyStream(options.htmlMinifierOptions || {})
            )
        );
      } else {
          ...
          try {
            Object.keys(partialsObject).forEach((name) => {
              engine.registerPartial(
                name,
                engine.compile(partialsObject[name])
              );
            });
  ...
  function viewLiquid(page, data, opts) {
    ...
    engine
      .renderFile(join(templatesDir, page), data, opts)
      .then((html) => {
        if (
          options.useHtmlMinifier &&
          typeof options.useHtmlMinifier.minify === "function"
